...
def run_migration_optimizer_integrated_mcf_arc(
    ...
    *,
    ...
    if allow_path_splitting:
        ...
        # ------------------------------------------------------------
        # (5b) Link continuous flow variable (f) with binary arc
        # selection variable (b_used).
        #
        # Purpose:
        #   f[s,c,u,v]      = amount of control traffic routed on arc (u,v)
        #   b_used[s,c,u,v] = 1 if arc (u,v) is used by commodity (s,c),
        #                     0 otherwise.
        #
        # These constraints ensure that b_used correctly reflects whether
        # an arc actually carries traffic. This is especially important
        # because the worst-path propagation delay (computed later using
        # node potentials pi) is based on b_used rather than on f.
        # ------------------------------------------------------------

        for (s, c) in commodity_pairs:

            # Total control traffic demand (bits/sec) generated by switch s.
            d_bits = float(demand_bits[s])
            ...
            for (u, v) in arcs:
                ...
                #     f <= 0
                #
                # and because flow variables are non-negative,
                #
                #     f = 0.
                #
                # Therefore, traffic cannot pass through an arc unless
                # that arc is selected.
                # --------------------------------------------------------
                m.addConstr(
                    f[s, c, u, v] <= d_bits * b_used[s, c, u, v],
                ...
                # Constraint 2:
                #
                # If b_used = 1  -->  flow must be strictly positive.
                #
                # Since:
                #
                #     f >= EPS_FLOW * b_used
                ...
                #     f >= EPS_FLOW.
                #
                # EPS_FLOW is a very tiny positive number (e.g. 1e-9),
                # which is practically zero but guarantees that an arc
                # cannot be marked "used" while carrying zero traffic.
                #
                # Without this constraint, Gurobi could choose:
                #
                #     b_used = 1
                #     f      = 0
                #
                # which would incorrectly make this arc contribute to the
                # worst-path propagation delay calculation.
                # --------------------------------------------------------
                m.addConstr(
                    f[s, c, u, v] >= EPS_FLOW * b_used[s, c, u, v],
                ...
                # Constraint 3:
                #
                # Arc selection is allowed only if commodity (s,c) is
                # actually active.
                #
                # Recall:
                #
                #     y[s,c] = 1  --> switch s is assigned to controller c
                #     y[s,c] = 0  --> switch s is NOT assigned to controller c
                #
                # Therefore,
                #
                #     b_used <= y
                ...
                #     b_used <= 0
                #
                # so every arc for this commodity must be inactive.
                #
                # If y = 1:
                #
                #     b_used may become either 0 or 1,
                #     allowing the optimizer to choose the routing path.
                #
                # This prevents creation of routing paths for
                # switch-controller pairs that are never selected.
                # --------------------------------------------------------
                m.addConstr(
                    b_used[s, c, u, v] <= y[s, c],
        ...
        # ------------------------------------------------------------
        # These constraints ensure that the traffic demand of every
        # switch-controller commodity (s,c) is routed correctly through
        # the network.
        #
        # A commodity (s,c) represents the control traffic generated by
        # switch 's' that must reach controller 'c'.
        #
        # Principle:
        # ----------
        # Traffic cannot magically appear or disappear.
        #
        # Therefore,
        #
        #   • Source switch injects traffic.
        #   • Destination controller absorbs traffic.
        #   • Intermediate nodes only forward traffic.
        #
        # This is the fundamental flow conservation law used in
        # Multi-Commodity Flow (MCF) optimization.
        # ------------------------------------------------------------

        for (s, c) in commodity_pairs:

            # Total traffic demand generated by switch s.
            #
            # Units:
            # bits/sec
            #
            # Example:
            # demand_bits[s] = 5000 bits/sec
            #
            # This is the amount of traffic that must travel from
            # switch s to controller c (if controller c is selected).
            d_bits = float(demand_bits[s])

            # Apply flow conservation at every node in the network.
            for n in G.nodes():

                # ----------------------------------------------------
                # Total outgoing traffic of commodity (s,c)
                # leaving node n.
                #
                # Example:
                #
                ...
                out_sum = gp.quicksum(
                    f[s, c, uu, vv]
                    for (uu, vv) in out_arcs[n]
                )

                # ----------------------------------------------------
                # Total incoming traffic of commodity (s,c)
                # entering node n.
                #
                # Example:
                #
                ...
                if n == s:
                    ...
                    # Source switch generates traffic.
                    #
                    # Therefore:
                    #
                    # outgoing flow
                    ...
                    # Mathematically:
                    #
                    # Σout f − Σin f = demand × y[s,c]
                    #
                    # If y[s,c]=1:
                    #
                    # Source injects the complete demand.
                    #
                    # If y[s,c]=0:
                    #
                    # RHS becomes zero,
                    # meaning no traffic is generated because
                    # controller c is not selected.
                    m.addConstr(
                        out_sum - in_sum == d_bits * y[s, c],
                ...
                # ----------------------------------------------------
                    ...
                    # Destination controller consumes traffic.
                    #
                    # Therefore:
                    #
                    # incoming flow
                    ...
                    # Mathematically:
                    #
                    # Σin f − Σout f = demand × y[s,c]
                    #
                    # If controller c is assigned,
                    # it must receive the complete demand.
                    #
                    # If not assigned,
                    # both sides become zero.
                    m.addConstr(
                        in_sum - out_sum == d_bits * y[s, c],
                ...
                # ----------------------------------------------------
                    ...
                    # Intermediate nodes neither create nor consume
                    # traffic.
                    #
                    ...
                    #      20 Mbps out
                    #
                    # No traffic is lost or generated.
                    m.addConstr(
                        out_sum - in_sum == 0.0,
        ...
        # ------------------------------------------------------------
        # (5') Link flow variable (f) with binary arc-selection
        # variable (b_used) for UNSPLITTABLE routing.
        #
        # In the unsplittable model, a switch-controller commodity
        # (s,c) must use exactly ONE path from switch s to controller c.
        #
        # Therefore:
        #
        #   • If an arc is selected (b_used = 1),
        #     the COMPLETE demand must flow through that arc.
        #
        #   • If an arc is not selected (b_used = 0),
        #     no flow can pass through that arc.
        #
        # Unlike the splittable model, traffic cannot be divided
        ...
        for (s, c) in commodity_pairs:
            ...
            # demand_bits[s] = 5000 bits/sec
            #
            # This complete demand must travel on the selected path.
            d_bits = float(demand_bits[s])

            # Examine every directed arc in the network.
            for (u, v) in arcs:

                # ----------------------------------------------------
                # Constraint 1:
                #
                # Flow equals full demand if the arc is selected.
                #
                # Mathematical form:
                #
                # f = demand × b_used
                ...
                # Example:
                #
                # demand = 5000 bits/sec
                #
                # Selected arc:
                #
                #      f = 5000
                #
                # Entire traffic flows through this arc.
                #
                # -----------------------------------------
                #
                # Case 2:
                ...
                #      f = 0
                #
                # No traffic is allowed on this arc.
                #
                # Therefore every selected arc carries the
                # complete commodity demand.
                #
                # This is correct because the commodity
                ...
                # Constraint 2:
                #
                # Arc selection is allowed only if controller c
                # is actually selected for switch s.
                #
                # Recall:
                #
                # y[s,c] = 1
                #      switch s is assigned to controller c
                #
                # y[s,c] = 0
                #      switch s is NOT assigned to controller c
                #
                # Therefore:
                #
                # b_used <= y
                ...
                #      b_used = 0
                #
                # so no routing path can be created.
                #
                # If y = 1:
                #
                # the optimizer is free to choose whether
                # this particular arc belongs to the unique path.
                #
                # Without this constraint,
                # Gurobi could create paths for
                # switch-controller pairs that are not assigned.
                # ----------------------------------------------------
                m.addConstr(
                    b_used[s, c, u, v] <= y[s, c],
        ...
        # ------------------------------------------------------------
        # (6') Single directed s->c path constraints on b_used
        # ------------------------------------------------------------
        # Purpose:
        #   Build exactly one path from switch s to controller c
        #   when y[s,c] = 1.
        #
        # b_used[s,c,u,v] = 1 means arc u -> v is selected
        # for routing traffic from switch s to controller c.
        #
        # This block prevents:
        #   - splitting into multiple paths
        #   - branching at intermediate nodes
        #   - paths entering the source
        #   - paths leaving the destination
        # ------------------------------------------------------------

        for (s, c) in commodity_pairs:

            # Check path behavior at every node n.
            for n in G.nodes():

                # Number of selected outgoing arcs from node n.
                #
                # Example:
                # if selected arcs from n are n->a and n->b,
                # then outb = 2.
                outb = gp.quicksum(
                    b_used[s, c, uu, vv]
                    for (uu, vv) in out_arcs[n]
                ...
                # Number of selected incoming arcs into node n.
                #
                # Example:
                # if selected arcs into n are a->n and b->n,
                # then inb = 2.
                inb = gp.quicksum(
                    b_used[s, c, uu, vv]
                    for (uu, vv) in in_arcs[n]
                ...
                if n == s:
                    # SOURCE NODE: switch s
                    #
                    # If y[s,c] = 1:
                    #   outb = 1
                    #   exactly one arc leaves the source.
                    #
                    # If y[s,c] = 0:
                    #   outb = 0
                    #   no path is created.
                    m.addConstr(
                        outb == y[s, c],
                        name=f"b_src_out_{s}_{c}_{n}"
                    )

                    # No selected arc should enter the source.
                    #
                    # This prevents paths like:
                    #   a -> s -> ...
                    m.addConstr(
                        inb == 0,
                        name=f"b_src_in_{s}_{c}_{n}"
                    )

                elif n == c:
                    # DESTINATION NODE: controller c
                    #
                    # If y[s,c] = 1:
                    #   inb = 1
                    #   exactly one arc enters the controller.
                    #
                    # If y[s,c] = 0:
                    #   inb = 0
                    #   no path is created.
                    m.addConstr(
                        inb == y[s, c],
                        name=f"b_sink_in_{s}_{c}_{n}"
                    )

                    # No selected arc should leave the destination.
                    #
                    # This prevents paths like:
                    #   ... -> c -> a
                    m.addConstr(
                        outb == 0,
                        name=f"b_sink_out_{s}_{c}_{n}"
                    )

                else:
                    # INTERMEDIATE NODE
                    #
                    # Intermediate nodes should only forward traffic.
                    #
                    # Either:
                    #   node is not used:
                    #       inb = 0, outb = 0
                    #
                    # Or:
                    #   node is used in the path:
                    #       inb = 1, outb = 1

                    # Flow/path continuity:
                    # if path enters node n, it must also leave node n.
                    m.addConstr(
                        outb == inb,
                        name=f"b_bal_{s}_{c}_{n}"
                    )

                    # At most one selected outgoing arc.
                    #
                    # Prevents branching:
                    #   n -> a
                    #   n -> b
                    m.addConstr(
                        outb <= 1,
                        name=f"b_outdeg1_{s}_{c}_{n}"
                    )

                    # At most one selected incoming arc.
                    #
                    # Prevents two paths merging into the same node.
                    m.addConstr(
                        inb <= 1,
                        name=f"b_indeg1_{s}_{c}_{n}"
                    )

    # ============================================================
    # (6b) WORST USED PATH DELAY using node potentials
    # ============================================================
    # This block computes the one-way propagation delay from switch s
    # to controller c using the selected arcs b_used.
    #
    # Important:
    # ----------
    # pi[s,c,n] is NOT the latency of one link.
    #
    # pi[s,c,n] means:
    #   accumulated delay from source switch s up to node n.
    #
    # arc_delay_sec[(u,v)] means:
    #   latency of only one directed arc u -> v.
    #
    # So if arc u -> v is used:
    #
    #   delay up to v = delay up to u + delay of arc u -> v
    #
    # This is why we use:
    #
    #   pi[v] >= pi[u] + arc_delay_sec[(u,v)]
    #
    # If multiple paths are used, pi at destination controller c
    # becomes at least the delay of every used path.
    # Therefore it represents the worst / longest used path delay.
    ...
    for (s, c) in commodity_pairs:
        ...
        # --------------------------------------------------------
        # The accumulated delay from source switch s to itself is zero.
        #
        # Example:
        # path starts at S
        # pi[S] = 0
        ...
        # --------------------------------------------------------
        # Propagate delay along selected arcs
        # --------------------------------------------------------
        for (u, v) in arcs:

            # If arc u -> v is used by commodity (s,c), then:
            #
            #   pi[s,c,v] >= pi[s,c,u] + arc_delay_sec[(u,v)]
            #
            # Meaning:
            #   accumulated delay up to v
            #   =
            #   accumulated delay up to u
            #   +
            #   latency of link u -> v
            #
            # Example:
            ...
            #   pi[B] >= 2 + 3 = 5
            #
            # So pi[B] becomes total delay from S to B.
            #
            # Big-M part:
            # -----------
            # If b_used[s,c,u,v] = 1:
            ...
            # If b_used[s,c,u,v] = 0:
            #
            #   BIGM_PI * (1 - b_used) = BIGM_PI
            #
            # so the constraint becomes relaxed:
            #
            #   pi[v] >= pi[u] + delay - BIGM_PI
            #
            # This makes unused arcs ignored.
            m.addConstr(
                pi[s, c, v]
                >= pi[s, c, u]
                + arc_delay_sec[(u, v)]
        ...
        # --------------------------------------------------------
        # Store worst one-way path delay
        # --------------------------------------------------------
        # At the destination controller c, pi[s,c,c] represents
        # the accumulated propagation delay from source switch s
        # to controller c.
        #
        # If there is only one selected path:
        #   Pworst_oneway_sec = delay of that path.
        #
        # If traffic is split over multiple paths:
        #   pi[s,c,c] is forced to be at least the delay of each
        #   used path, so it represents the slowest / worst path.
        #
        # Later RTT is computed as:
        #
        #   RTT_flow_sec[s,c] = 2 * Pworst_oneway_sec[s,c]
        #
        # when round_trip=True.
        m.addConstr(
    ...
    for c in controllers:
         muc = float(mu_eff[c]) 
        #  muc-controller capacity in req/s
        if muc <= 0.0:
